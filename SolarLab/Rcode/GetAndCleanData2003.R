...
library(lubridate)
Cam2003<- read.table("../data/raw/project_label_BSRN_event_label_CAM_2003.tab",sep="\t", header=TRUE,stringsAsFactors=FALSE)
str(Cam2003)
summary(Cam2003)
names(Cam2003)<-c("datetime","DIF","LWD","SWD","DIR","ORG")

# convert time to POSIXct
Cam2003$datetime<-ymd_hms(Cam2002$datetime)
# remove last column
Cam2003$ORG<-NULL
...
plot(t[daybegin:dayend],Cam2003$SWD[daybegin:dayend],type="l",ylim=c(0,500))
lines(t[daybegin:dayend],Q[daybegin:dayend],type="l",col="blue")

plot(Cam2003$datetime[daybegin:dayend])
Cam2003$datetime[(daybegin+270):(daybegin+280)]

# Save data
######################################################################
saveRDS(Cam2003, "../data/cleaned/Cam2003n.csv")
rm(Cam2003)